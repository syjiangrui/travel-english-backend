...
Go Backend
    ...
    └── POST /tts      — 独立 ElevenLabs TTS 合成 (MP3)
```
...
| `tts.synthesize.full` | 纯 TTS 合成别名（兼容旧客户端） | `text` |
...
基于“已有记忆 + 最近对话”生成完整的长期记忆快照。客户端每 2 轮 AI 回复后调用，通常会携带最近 8 条消息；不足 8 条时则携带当前已有的全部消息。
...
{
  "memories": [
    "用户喜欢户外活动，最近在学登山，计划下周去爬黄山",
    "用户的职业是医生，平时工作繁忙"
  ]
}
```

LLM 参数：`temperature: 0.1`（稳定 JSON 输出）、`max_tokens: 300`。服务端职责：合并旧记忆、吸收新对话、去重、用新信息覆盖旧信息、返回完整快照。解析策略：正则 `\[.*?\]` 提取 → 直接 JSON parse → 返回空数组。

### POST /tts

对单段文本做独立 TTS 合成，直接返回 `audio/mpeg` 响应体。适合跟读面板“标准发音”、消息重播等不想走完整对话状态机的场景。

```json
// 请求
{
  "text": "Could you help me find my gate?",
